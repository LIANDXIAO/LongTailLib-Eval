...
class Server(object):
    ...
    def save_results(self):
        ...
        if len(self.rs_test_acc):
            ...
            with h5py.File(file_path, 'w') as hf:
                ...
                if len(self.rs_acc_var):
                    hf.create_dataset('rs_acc_var', data=self.rs_acc_var)
            ...
            # Generate and save Tail Class Accuracy curve
            try:
                if len(self.rs_tail_acc):
                    t_rounds = list(range(len(self.rs_tail_acc)))
                    plt.figure(figsize=(7, 4), dpi=150)
                    plt.plot(t_rounds, self.rs_tail_acc, marker='^', linewidth=1.8, markersize=3, color='tab:purple', label='Tail Class Test Accuracy')
                    plt.xlabel('Round')
                    plt.ylabel('Tail Test Accuracy')
                    plt.title(f'{self.dataset}-{self.algorithm}: Tail Test Accuracy per Round')
                    plt.grid(True, alpha=0.3)
                    plt.legend()
                    if len(t_rounds) <= 20:
                        plt.xticks(t_rounds)
                    else:
                        plt.xticks(list(range(0, t_rounds[-1] + 1, max(1, len(t_rounds) // 20))))
                    plt.tight_layout()
                    t_svg_path = os.path.join(run_dir, f"{base_name}_tail.svg")
                    plt.savefig(t_svg_path, format='svg')
                    plt.close()
                    print("Saved Tail Class Accuracy curve: " + t_svg_path)
            except Exception as e:
                print("Warning: failed to save Tail Class Accuracy curve:", e)
                
            # Generate and save Inter-class Accuracy Variance curve
            try:
                if len(self.rs_acc_var):
                    v_rounds = list(range(len(self.rs_acc_var)))
                    plt.figure(figsize=(7, 4), dpi=150)
                    plt.plot(v_rounds, self.rs_acc_var, marker='d', linewidth=1.8, markersize=3, color='tab:orange', label='Inter-class Acc Variance')
                    plt.xlabel('Round')
                    plt.ylabel('Variance')
                    plt.title(f'{self.dataset}-{self.algorithm}: Accuracy Variance per Round')
                    plt.grid(True, alpha=0.3)
                    plt.legend()
                    if len(v_rounds) <= 20:
                        plt.xticks(v_rounds)
                    else:
                        plt.xticks(list(range(0, v_rounds[-1] + 1, max(1, len(v_rounds) // 20))))
                    plt.tight_layout()
                    v_svg_path = os.path.join(run_dir, f"{base_name}_variance.svg")
                    plt.savefig(v_svg_path, format='svg')
                    plt.close()
                    print("Saved Accuracy Variance curve: " + v_svg_path)
            except Exception as e:
                print("Warning: failed to save Accuracy Variance curve:", e)
    ...
    def compute_global_test_accuracy(self):
        ...
        Returns a dict containing 'global_acc', 'tail_acc', 'acc_var'
        """
        ...
        try:
            ...
            class_acc = class_correct / torch.clamp(class_total, min=1e-5)
            class_acc_np = class_acc.cpu().numpy()
            
            acc_var = float(np.var(class_acc_np))
            tail_split = int(self.num_classes * 0.66) # Last ~34% classes
            tail_acc = float(np.mean(class_acc_np[tail_split:]))
    ...
    def evaluate(self, acc=None, loss=None):
        ...
        g_metrics = self.compute_global_test_accuracy()
        if g_metrics is not None:
            self.rs_global_acc.append(g_metrics['global_acc'])
            self.rs_tail_acc.append(g_metrics['tail_acc'])
            self.rs_acc_var.append(g_metrics['acc_var'])
            print("Global Averaged Test Accuracy: {:.4f}".format(g_metrics['global_acc']))
            print("Tail Class Average Accuracy: {:.4f}".format(g_metrics['tail_acc']))
            print("Inter-class Accuracy Variance: {:.4f}".format(g_metrics['acc_var']))
        else:
            # If global accuracy not available, use local accuracy as fallback  
            self.rs_global_acc.append(test_acc)
            self.rs_tail_acc.append(0.0)
            self.rs_acc_var.append(0.0)
