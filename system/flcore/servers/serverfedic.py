...
from system.flcore.clients.clientfedic import ClientFEDIC
from system.flcore.servers.serverbase import Server
from system.utils.data_utils import read_client_data
from system.flcore.trainmodel.resnet_cifar import resnet8_cifar
    ...
    def train(self):
        ...
        for round_idx in range(1, self.global_rounds + 1):
            ...
            if round_idx % self.eval_gap == 0:
                ...
                print("\nEvaluate global model")
                # Send updated model to clients for evaluation
                for client in self.clients:
                    client.set_parameters_dict(self.dict_global_params)
                self.evaluate()
                if len(self.rs_global_acc) > 0:
                    self.epoch_acc.append(self.rs_global_acc[-1])
